...
const activeFilterChips = computed(() => {
  ...
  if (statusFilter.value !== 'ALL') {
    const statusLabels = {
      ACTIVE: 'En transcurso',
      INACTIVE: 'Completado',
      NEW: 'Nuevo'
    }
    chips.push({ label: 'Estado', value: statusLabels[statusFilter.value] || statusFilter.value, field: 'statusFilter' })
  }
  return chips
})
...
const removeFilter = (field) => {
  ...
  else if (field === 'statusFilter') statusFilter.value = 'ALL'
}
...
<template>
  <div class="h-screen min-h-0 overflow-hidden bg-background font-sans flex flex-col">
    ...
    <div class="flex flex-1 min-h-0 overflow-hidden">
      ...
      <main class="flex-1 min-h-0 pl-4 pr-4 sm:pr-6 lg:pr-8 pt-4 pb-10 overflow-y-auto flex flex-col min-w-0">
        <div class="flex gap-6 w-full min-w-0 max-w-full">
          ...
          <div v-if="!selectedLocationForMap || !canShowDestinationMap" :class="[
            ...
            <!-- Header Tabla -->
            <div class="shrink-0 px-4 sm:px-8 md:pr-10 pt-4 pb-4 sm:pb-6">
            ...
            <!-- Error -->
            <div v-if="loadError" class="bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-lg mb-4 mx-8 font-body shrink-0">
              {{ loadError }}
            </div>
            ...
            <div class="px-8 md:px-12 overflow-x-auto mb-4">
              <table class="w-full min-w-[920px] table-fixed border-collapse">
                ...
                <tbody>
                  ...
                  <tr v-for="dest in pagedDestinations" :key="dest.id" v-else class="bg-white hover:bg-slate-50/60 transition-colors">
          ...
          <!-- MAP VIEW CARD -->
          <div v-else-if="canShowDestinationMap" class="bg-white rounded-[2rem] border-2 border-slate-300 shadow-sm flex-1 flex flex-col overflow-visible transition-all duration-300 relative w-full p-8 hidden-scroll">
      ...
      <transition enter-active-class="transition duration-150 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="isCreateModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div>
              <label class="block text-sm font-titles font-semibold text-slate-600 mb-1">Nombre</label>
              <input v-model="createForm.name" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-body focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="Nombre del destino" @keyup.enter="saveCreate" />
            </div>
